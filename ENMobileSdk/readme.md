...
```gradle
dependencies {
  implementation 'com.euronovate.mobilesdk:core:1.3.20'
}
```
...
class ENMobileSdkConfig(  
    ...
    var defaultSignatureMode: SignatureMode = SignatureMode.GraphometricFea,
)
```
...
```kotlin
sealed class SignatureMode(val acquireSignatureAdditionalDataName: String) {

    companion object {
        const val DEFAULT_PINPAD_NUMBER_OF_DIGITS = 4
        const val DEFAULT_PINPAD_DEFAULT_HIDDEN_DIGITS = true

        fun fromString(value: String): SignatureMode? {
            return if (value.equals("FEA", ignoreCase = true)) {
                GraphometricFea
            } else if (value.equals("FES", ignoreCase = true)) {
                GraphometricFes
            } else if (value.equals("OTP", ignoreCase = true)) {
                OtpByDocumentGuid()
            } else {
                null
            }
    ...
    data object GraphometricFea : SignatureMode(acquireSignatureAdditionalDataName = "FEA")

    data object GraphometricFes : SignatureMode(acquireSignatureAdditionalDataName = "FES")
    ...
    ) : SignatureMode(acquireSignatureAdditionalDataName = "FES") // NOTE: the OtpByDocumentGuid is a FES type of signature
}
```
...
ENMobileSDK.emitEvent(
    ...
    ENSignDocument(
        ...
        documentSignatureMode = SingatureMode.GraphometricFea,
    )
)
```
