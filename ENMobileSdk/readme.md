...
* `certificateIntegrity`: is a sealed with 3 options:

1) `P12(val certificateBase64: String, val password: String)`
2) `Pem(val certificateBase64: String)`
3) `Autogenerated`

With **P12 & Pem** user has to provide a base64 with privateKey and certificate. If it is set, the certificate generation operation will be skipped during the applying of the first signature in the document.
